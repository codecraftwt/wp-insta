...
@section('content')
    ...
    <script>
        $(document).ready(function() {
            // Function to renew or buy a subscription plan
            function renewOrBuyPlan(currentEndDate, duration, subscriptionType, price, plainId) {
                // Show loader modal before making the request
                $('#loaderModal').modal('show');
            ...
            // Fetch current and available plans
            $.ajax({
                url: '/renew-plans-data',
                method: 'GET',
                dataType: 'json',
                ...
                `);
                    ...
                    // Sort available plans: "Renew" plans first
                    const renewPlans = [];
                    const otherPlans = [];

                    data.membershipPlans.forEach(plan => {
                        const isCurrentPlan =
                            data.currentPlan &&
                            data.currentPlan.subscription_type === plan.plain_title &&
                            data.currentPlan.duration === plan.plan_type;
                        ...
                        if (isCurrentPlan) {
                            renewPlans.push(plan);
                        } else {
                            otherPlans.push(plan);
                        }
                    });

                    const sortedPlans = [...renewPlans, ...otherPlans];
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                <h2 class="card-title text-center">Price: ₹${plan.plan_price}</h2>
                                <h5 class="card-title text-center">Duration: ${plan.plan_type}</h5>
                                <div>${plan.plan_details}</div>
                            </div>
                            <div class="card-footer text-center">
                                <button class="btn btn-${isCurrentPlan ? 'primary' : 'success'}"
